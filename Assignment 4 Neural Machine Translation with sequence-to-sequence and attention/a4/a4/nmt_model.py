...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        X_unpacked = self.model_embeddings.source(source_padded)
        X = pack_padded_sequence(X_unpacked, source_lengths)
        enc_hiddens_unpadded, (last_hidden, last_cell) = self.encoder(X)
        enc_hiddens, _ = pad_packed_sequence(enc_hiddens_unpadded, batch_first=True)
